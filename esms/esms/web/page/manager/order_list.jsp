...
<body>
						...
						<c:if test="${orderList.size() > 0}">
							<c:forEach items="${orderList}" var="arr" varStatus="status">
								<tr>
									...
									<td>
										<button onclick="pass('${arr.id}')" type="button"
											class="btn btn-success btn-sm">同意</button>
										<button onclick="refuse('${arr.id}')" type="button"
											class="btn btn-danger btn-sm">拒绝</button>
									</td>
								</tr>
							</c:forEach>
						</c:if>
	...
	<script type="text/javascript">
		...
		function jump() {
			...
			var page = $("#page").val();
			window.location.href = "manager/order_list?goodsName="+goodsName+"&page="+page;
		}
		
		function pass(id) {
			if (window.confirm("确认是否通过？")) {
				//通过ajax删除
				$.ajax({
					url: "manager/order_pass",
					type: "POST",
					dataType: "text",
					data: {
						id: id
		...
		function refuse(id) {
			if (window.confirm("确认是否拒绝？")) {
				//通过ajax删除
				$.ajax({
					url: "manager/order_refuse",
					type: "POST",
					dataType: "text",
					data: {
						id: id
